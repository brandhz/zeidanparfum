...
/* =====================================================
   COMPARTILHAMENTO BLINDADO (ZEIDAN PARFUM)
   ===================================================== */
const btnShare = document.getElementById('btnCompartilhar');

if (btnShare) {
    btnShare.addEventListener('click', async () => {
        // Dados dinâmicos (pega o título da página atual)
        const shareData = {
            title: document.title || "Zeidan Parfum",
            text: 'Olha esse perfume que achei na Zeidan:', // Texto ajustado para perfume
        ...
        // 2. SE FOR PC OU NÃO TIVER SHARE, TENTA COPIAR (MÉTODO RESILIENTE)
        else {
...
// --- FUNÇÕES AUXILIARES (COPIAM MESMO SEM HTTPS) ---

function copiarLinkResiliente(texto) {
    // Tenta o método moderno primeiro (só funciona em HTTPS)
...
function tentarMetodoAntigo(texto) {
    ...
    try {
        const successful = document.execCommand('copy'); // O comando clássico que nunca falha
